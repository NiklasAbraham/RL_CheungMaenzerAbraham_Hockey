...
def plot_training_metrics(
    ...
    opponent_metrics = [
        k for k in metric_names if "opponent" in k.lower() and "cloning" in k.lower()
    ]
    ...
    if rewards:
        ...
        _add_rolling_percentile_bands(
            axes[ax_idx], episodes, rewards, window=min(500, len(rewards))
        )
        axes[ax_idx].set_xlabel("Episode")
        axes[ax_idx].set_ylabel("Reward")
        axes[ax_idx].set_title("Reward per Episode")
    ...
    for i, metric_name in enumerate(other_metrics):
        ...
        if values:
            ...
            axes[ax_idx].plot(
                steps, values, alpha=0.3, label="Raw", color=colors_main[i]
            )
            axes[ax_idx].plot(
                steps,
                mov,
    ...
    if opponent_metrics and ax_idx < len(axes):
        ...
        for i, metric_name in enumerate(opponent_metrics):
            ...
            if values:
                mov = _moving_average(values, window_size)
                opponent_label = metric_name.replace("opponent_", "Opponent ").replace(
                    "_cloning_loss", ""
                )
