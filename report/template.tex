...
\subsection{REDQ-TD3 Hyperparameters and Curriculum}

We followed the default hyperparameters from the REDQ paper: UTD Ratio $G=20$, Ensemble Size $N=10$, Subset Size $M=2$, and Learning Rate $3 \times 10^{-4}$. For the Actor and Critics, we followed TD3's paper, 2 layer network with hidden dim 256 and ReLU non-linear activation for Actor and tanh non-linear activation for Critics. Gradient clipping and Layer Norm was added to stabilize training as well.
...
Figure~\ref{fig:reward_sparse_overview} shows the outcome: regardless of
whether an episode ends in a player-1 win, player-2 win, or draw, the
per-step rewards are overwhelmingly clustered at zero.  A thin spike at
$+10$ (or $-10$) appears only for the single terminal step of decided
episodes; the rest of each episode contributes essentially nothing to
the value-function learning signal.

\begin{figure}[h]
    \centering
    \includegraphics[width=\textwidth]{figures/reward_sparse_overview.png}
    \caption{%
        Reward distribution across 100 random-play episodes, separated by
        outcome.
        \textbf{Left:} per-step reward density; almost all steps yield
        rewards near zero.
        \textbf{Right:} episode total reward; winning episodes receive their
        entire positive return from the single terminal $+10$ step.
    }
    \label{fig:reward_sparse_overview}
\end{figure}
