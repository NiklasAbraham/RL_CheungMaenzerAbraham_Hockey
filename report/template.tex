...
TD-MPC2 learns a latent representation (rather than raw observations) to estimate action-sequence outcomes. Five neural network components interact as in Figure~\ref{fig:tdmpc2_flow} a).
\begin{figure}[h]
    \centering
...
Classical TD-MPC2 dynamics use only latent state and self-action, so the model implicitly marginalizes over the opponent:
\begin{equation}
...
A separate network $\pi_{\text{opp}}$ predicts the opponent action, trained by imitation on replay-buffer actions (Figure~\ref{fig:tdmpc2_flow} b). We train it against weak/strong bots, TD3, SAC, and TD-MPC2; $a_t^{\text{opp}}$ is recorded exactly during collection. The loss is
\begin{equation}
\mathcal{L}_{\text{opp}} = \| a_t^{\text{opp}} - \pi_{\text{opp}}(\vec{z}_t) \|^2,
\end{equation}
$\pi_{\text{opp}}$ is trained with a frozen encoder periodically; MSE yields a deterministic mean predictor per latent state.

At inference, $\hat{a}_t^{\text{opp}} = \pi_{\text{opp}}(\vec{z}_t)$ is fed into the dynamics. With multiple opponent models, each MPPI rollout uses a fixed model over the horizon.
...
\textbf{Exploration Strategies:} To evaluate exploration, we compared standard white noise against temporally correlated pink and red noise, as shown in Figure \ref{fig:noise_plot}. The results demonstrate that white noise achieved slightly better cumulative rewards and a higher overall win rate while requiring fewer total steps, indicating superior sample efficiency. Consequently, we retained white noise, relying on SAC's entropy tuning to regulate exploration.

\begin{figure}[t]
    \centering
    \includegraphics[width=\linewidth]{figures/noise_plot.png}
    \caption{Cumulative reward and win rate across different exploration noise types over the first 10,000 episodes against the strong bot (100-episode moving average; raw rewards in the background).}
...
The second trick of TD3 is \textbf{Target Policy Smoothing}. In continuous action spaces, the Critics have a rough Q-value landscape laced with sharp peaks.This causes the Critics to overestimate the Q-value and the Actor exploits these peaks. Hence, TD3 injects small random noise into the Actor's predicted action when updating the Critics.
    ...
    \end{equation}\label{clipped_action}
...
Trick 2: \textbf{Ensemble Minimum}. In order to continue to fix the overestimation problem, REDQ uses an even more pessimistic estimation. REDQ maintains an ensemble of $N$ Q-functions (Critics). For each update, using a minimum of all Critics might cause large overestimation bias. Instead, a random subset $\mathcal{M}$ of size $M$ is sampled to compute the target. $\theta$ is the policy parameters, $\phi_i, i=1, \dots, N$ is Q-function parameters.
    \begin{equation}
        y = r + \gamma \min_{j \in \mathcal{M}} Q_{\phi_{j, targ}}(s', \tilde{a}')
    \end{equation}

where $\tilde{a}'$ is the same clipped action from equation 16 (\ref{clipped_action}). This single target is used to update all $N$ critics. Using $N$ Critics with different initializations allows the approximation error to be even more uncorelated compared to TD3 with 2 Critics.
...
Trick 3: \textbf{Policy Update}. The actor is updated to maximize the mean Q-value across all $N$ Critics. The mean of all $N$ Critics provides a much smoother and reliable signal for the Actor to follow.
    \begin{equation}
        \nabla_\theta \frac{1}{|B|}\sum_{s\in B} \frac{1}{N} \sum_{i=1}^N Q_{\phi_i}(s, \pi_\theta(s))
    \end{equation}
...
To compare all trained agents on a common scale, we evaluated them in a round-robin tournament within the archive matchmaking system using TrueSkill \cite{herbrich2006trueskill}, a Bayesian rating algorithm that updates the belief over each agent's skill after every match (see \ref{sec:trueskill}). We report $\mu - 3\sigma$ as a conservative lower bound. Figure~\ref{fig:agent_ratings_selected} shows TrueSkill ratings and win rates against fixed baselines for the two scripted bots (weak and strong), the best SAC, TD-MPC2, and REDQ-TD3 agents.
...
All three learned agents consistently beat both bots, with win rates from $86\%$ to $99\%$. SAC has the highest rating ($37.32$, 391 matches), TD-MPC2 next at $29.90$ (16\,000 steps), and REDQ-TD3 at $21.01$, close to the scripted baselines (weak bot $22.46$, strong bot $20.06$).

Win rates confirm the rankings: SAC $99\%$/ $96\%$ (weak/strong bot), TD-MPC2 $98\%$/ $90\%$, REDQ-TD3 $86\%$/ $98\%$. REDQ-TD3 fares better against the strong bot's aggression than the weak bot's passive style. Overall, SAC is strongest, TD-MPC2 (16\,000 steps, $H{=}8$) is runner-up, with a clear margin over REDQ-TD3 and the scripted baselines.
...
\subsection{Curriculum TD-MPC2 Mixed Opponents: Hyperparameters}\label{sec:curriculum_tdmpc2_hyperparams}

The curriculum training run \texttt{curriculum\_tdmpc2\_mixed\_opponents} uses the configuration given in \texttt{configs/curriculum\_tdmpc2\_mixed\_opponents.json}. Table~\ref{tab:curriculum_tdmpc2_hyperparams} lists all training and agent hyperparameters. Table~\ref{tab:curriculum_tdmpc2_phases} summarizes the curriculum phases.
...
\multicolumn{2}{l}{\textit{Training (global)}} \\
...
Warmup steps & 5000 \\
Reward scale & 1 \\
Checkpoint save frequency & 200 \\ \midrule
\multicolumn{2}{l}{\textit{TD-MPC2 agent}} \\
Latent dimension & 256 \\
Encoder hidden dims & [256, 256, 256] \\
Dynamics hidden dims & [256, 256, 256] \\
Reward hidden dims & [256, 256, 256] \\
Termination hidden dims & [256, 256] \\
Q-function hidden dims & [256, 256, 256] \\
Policy hidden dims & [256, 256, 256] \\
Number of Q-networks ($Q$-ensemble) & 5 \\
Discount factor ($\gamma$) & 0.99 \\
Planning horizon $H$ & 8 \\
MPC samples & 512 \\
MPC iterations & 6 \\
Temperature & 0.5 \\
Value clip $V_{\min}$ & $-10$ \\
Value clip $V_{\max}$ & 10 \\
Win reward bonus & 10 \\
Win reward discount & 0.92 \\ \midrule
\multicolumn{2}{l}{\textit{Opponent simulation}} \\
Enabled & true \\
Cloning frequency & 2500 \\
Cloning steps & 5000 \\
Cloning samples & 512 \\
\bottomrule
\end{tabular}
\caption{Hyperparameters for the curriculum TD-MPC2 mixed opponents run.}
\label{tab:curriculum_tdmpc2_hyperparams}
\end{table}
...
\begin{tabular}{@{}llc@{}}
\toprule
\textbf{Phase} & \textbf{Episodes} & \textbf{Opponent type} \\ \midrule
initial\_training & 3000 & basic\_strong \\
mixed\_opponents & 7000 & weighted\_mixture (SAC, TDMPC2, DecoyPolicy, basic\_strong) \\
archive\_opponents\_1 & 2000 & archive \\
checkpoints\_1 & 2000 & run\_checkpoints (TDMPC2) \\
archive\_opponents\_2 & 2000 & archive \\
checkpoints\_2 & 2000 & run\_checkpoints (TDMPC2) \\
archive\_opponents\_3 & 2000 & archive \\
checkpoints\_3 & 2000 & run\_checkpoints (TDMPC2) \\
archive\_opponents\_4 & 5000 & archive \\
checkpoints\_4 & 5000 & run\_checkpoints (TDMPC2) \\
checkpoints\_5 & 5000 & run\_checkpoints (TDMPC2) \\
archive\_opponents\_5 & 5000 & archive \\
checkpoints\_6 & 50000 & run\_checkpoints (TDMPC2) \\
\bottomrule
\end{tabular}
\caption{Curriculum phases for the TD-MPC2 mixed opponents run. Environment mode is NORMAL for all phases.}
\label{tab:curriculum_tdmpc2_phases}
\end{table}
